...
for(S in 1:nperms){
  ...
  params.opt.fit <- optim(params.opt,c.CNDRspace.objective,control = ctrl, lower=c(10e-7,10e-7), # optimization. c's must be > 0
                          ...
                          Xo=Xo,ABA.to.CNDR.key=ABA.to.CNDR.key,fxn =scipy.linalg$expm,one.lm=T) # static inputs
...
save(seed.fits,c.retro.altseed,c.antero.altseed,alt.seed.distances,alt.seed.sites, file = paste(savedir,grp,'AlternateSeedFits_OneLM.RData',sep=''))
